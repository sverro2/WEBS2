...
class users extends controller { 
	...
	private $salt = "test01";		//used to create hashes that are more complicated
	...
	function login()
	{
		...
		if( count( $_POST ) ){
			...
			$user_data = $user->find_by_email_and_password( $_POST["email"], sha1($_POST["password"] . $this->salt)); //ddx )
	...
	function register()
	{
		...
		if( count( $_POST ) ){
			$email      = $_POST["email"];
			$password   = sha1($_POST["password"] . $this->salt); //ddx
